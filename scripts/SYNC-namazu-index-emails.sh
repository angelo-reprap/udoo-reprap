...
BRANCH="${BRANCH:-origin/cursor/posteingang-radar-fix-1532}"
LIVE_CMD="${LIVE_CMD:-/opt/abpe/backend/apps/namazu/management/commands}"

cd "$REPO"
git fetch origin cursor/posteingang-radar-fix-1532 || true
...
echo "  bash <(git show origin/cursor/posteingang-radar-fix-1532:scripts/SYNC-abpe-shaduler-files.sh)"
...
echo "  bash <(git show origin/cursor/posteingang-radar-fix-1532:scripts/ENSURE-abpe-scheduler-loop.sh)"
