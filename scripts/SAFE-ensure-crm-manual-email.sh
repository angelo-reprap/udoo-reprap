#!/usr/bin/env bash
# Email-Studio: crm_manual_email von origin einspielen.
#
# Erfolg: OK — crm_manual_email (Hülle / ohne steifen Schluss)
#
set -euo pipefail
...
REL="scripts/ensure-crm-manual-email-template.py"
...
echo "→ Upsert crm_manual_email …"
git -C "$REPO" show "origin/${BRANCH}:${REL}" | "$PY" manage.py shell
echo "Danach: Browser Ctrl+F5, Email Studio Vorlage crm_manual_email"
