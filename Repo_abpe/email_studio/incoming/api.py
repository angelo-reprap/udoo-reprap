...
class VariableListAPI(LoginRequiredMixin, View):
    """
    Gibt bekannte Variablen zurück — Single Source: variables_registry.

    Query: ?scope=telefon&identifier=meetme_invite_standard
    """

    def get(self, request):
        from .variables_registry import get_variables, get_sidebar_variable_groups

        scope = (request.GET.get('scope') or 'general').strip() or 'general'
        identifier = (request.GET.get('identifier') or '').strip() or None
