...
## Phase 2 — KI-Wizard UI (2026-07)

**Branch:** `cursor/email-studio-ki-wizard-phase2-bf44`  
**Voraussetzung:** `abpe_ki_wiz` Phase 1 (`/ki-wizard/api/health/` → `"phase": 1`)

### Repo vorher sichern (ucs5 → Repo)

```bash
cd /mnt/public/udoo-reprap && git pull
bash Repo_abpe/email_studio/incoming/RUN-sync-from-ucs5.sh
git add Repo_abpe/email_studio Repo_abpe/abpe_ki_wiz && git commit -m "sync ucs5"
```
...
```bash
cd /mnt/public/udoo-reprap
git fetch origin cursor/email-studio-ki-wizard-phase2-bf44
bash Repo_abpe/email_studio/incoming/DEPLOY-phase2-ki-wizard.sh
```

### Neue Dateien

| Repo (`incoming/`) | Live (ucs5) |
|---|---|
| `studio.html` | `apps/abpe_ui/templates/.../studio.html` |
| `ki-wizard-modal.html` | `apps/abpe_ui/templates/.../ki-wizard-modal.html` |
| `es-ki-wizard.js` | `apps/abpe_email_studio/static/email_studio/js/` |
| `es-studio.js` | (gleicher Pfad) |
| `mod-email_studio.css` | `apps/abpe_ui/static/abpe_ui/css/mod/` |
| `i18n/{lang}/email_studio.json` | `apps/abpe_ui/static/abpe_ui/i18n/{lang}/modules/email_studio/` |

### UI-Flow

1. `/email-studio/studio/` → Karte **KI-Assistent**
2. Modal: Briefing → Klärfragen → Metadaten → Vorschau
3. **In Editor übernehmen** → leere Vorlage mit HTML/TXT + Feldern
4. Live-Vorschau + Speichern wie gewohnt

i18n: alle Texte über `data-i18n="es.ki_*"` / `window.i18nData.es.*`  
Theme: CSS-Variablen aus `core-theme.css` (`var(--bg-white)`, `var(--border-color)`, …)
