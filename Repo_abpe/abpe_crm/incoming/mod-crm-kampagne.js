...
const KampagneUI = {
    ...
    async load(page) {
        ...
        if (list) list.innerHTML = '<div class="crm-list-loading"><i class="bi bi-hourglass-split"></i> Lade…</div>';
        ...
        try {
            ...
            if (list) list.innerHTML = '<div class="crm-list-loading">Fehler beim Laden</div>';
        }
    },
    ...
    _render(rows) {
        const list = document.getElementById('kamp-list');
        if (!list) return;
        if (!rows.length) { list.innerHTML = '<div class="crm-list-loading">Keine Treffer</div>'; return; }
        list.innerHTML = rows.map(r => {
            const checked  = this._selected.has(r.email_id) ? 'checked' : '';
            const kampIcon = r.kampagne_ok
                ? '<i class="bi bi-check-circle-fill" style="color:var(--badge-success-text);font-size:12px" title="Kampagne erlaubt"></i>'
                : '<i class="bi bi-x-circle" style="color:var(--text-muted);font-size:12px" title="Kampagne gesperrt"></i>';
            const typBadge = r.typ === 'berater'
                ? '<span class="crm-badge crm-badge-passiv">Berater</span>'
                : '<span class="crm-badge" style="background:#e0f2fe;color:#0369a1">Kunde</span>';
            return `<div class="crm-list-item" style="gap:8px">
                ...
                <div class="crm-item-right" style="gap:3px;align-items:flex-end">
                    ${typBadge}
                    <div style="display:flex;align-items:center;gap:3px">${kampIcon}<span style="font-size:10px;color:var(--text-muted)">${r.status||''}</span></div>
                </div>
            </div>`;
        }).join('');
    },
    ...
    _updatePagination(d) {
        ...
        if (info) info.textContent = `Seite ${d.page||1} von ${d.pages||1} · ${d.total||0} Einträge`;
    ...
    _updateSelectionUI() {
        ...
        if (info)     info.textContent = window.t('email_kampagne_ausgewaehlt').replace('{n}', n);
        if (badge)    badge.textContent = window.t('email_kampagne_ausgewaehlt').replace('{n}', n);
        if (btn)      { btn.disabled = n === 0; btn.style.opacity = n > 0 ? '1' : '.5'; }
        if (dlgCount) dlgCount.textContent = window.t('email_kampagne_empfaenger').replace('{n}', n);
        if (label)    label.textContent = window.t('email_kampagne_emails_senden').replace('{n}', n);
    },
    ...
    async send() {
        ...
        if (!template) { alert(window.t('email_kampagne_vorlage_waehlen')); return; }
        if (!sender)   { alert(window.t('email_kampagne_absender_waehlen')); return; }

        const btn = document.getElementById('kamp-send-confirm');
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="bi bi-hourglass-split"></i> ' + window.t('email_kampagne_sende'); }
        ...
        try {
            ...
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-send"></i> Senden'; }
        }
    },
    ...
    _showLog(d) {
        ...
        if (stats) stats.innerHTML = [
            ['Gesendet', results.length, ''],
            ['Erfolgreich', ok, 'color:var(--badge-success-text)'],
            ['Fehler', err, 'color:var(--badge-error-text)'],
        ...
        if (logList) logList.innerHTML = '<table style="width:100%;font-size:12px;border-collapse:collapse">' +
            ...
            '<th style="padding:7px 12px;text-align:left;font-weight:500;color:var(--text-muted)">Name</th>' +
            '<th style="padding:7px 4px;text-align:left;font-weight:500;color:var(--text-muted)">E-Mail</th>' +
            '<th style="padding:7px 12px;text-align:center;font-weight:500;color:var(--text-muted)">Status</th>' +
            '</tr></thead><tbody>' +
            results.map(x => `<tr style="border-bottom:1px solid var(--border-light)">
                ...
                <td style="padding:7px 12px;text-align:center">
                    ${x.ok
                        ...
                        : '<span class="crm-badge crm-badge-error" title="'+( x.error||'')+'">Fehler</span>'}
                </td>
            </tr>`).join('') +
            '</tbody></table>';
