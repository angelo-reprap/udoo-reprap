...
const CRM_Berater = {
    ...
    renderDetail(d) {
        ...
        panel.innerHTML =
            ...
            '<div class="crm-avatar" style="width:46px;height:46px;cursor:pointer;position:relative;overflow:hidden;border-radius:50%" onclick="CRM_Berater.avatarClick(\'' + crm_id + '\',\'' + (hasPhoto?'1':'0') + '\')" title="' + (hasPhoto ? 'Foto ändern oder löschen' : 'Foto hochladen') + '">' +
            avatarContent +
            ...
            '<span class="crm-detail-name-part" data-field="salutation" data-value="' + (d.salutation || '') + '" ondblclick="CRM_Edit.inlineEdit(this,\'' + crm_id + '\',\'salutation\')" style="cursor:pointer;border-bottom:1px dashed rgba(255,255,255,.4)">' + (d.salutation || 'Anrede') + '</span>' +
            ...
            '<span class="crm-badge ' + badgeCls + '" style="cursor:pointer" ondblclick="CRM_Edit.inlineEdit(this,\'' + crm_id + '\',\'kontakt_status_c\',\'select\',[{value:\'aktiv\',label:\'Aktiv\'},{value:\'passiv\',label:\'Passiv\'},{value:\'unbekannt\',label:\'Unbekannt\'}])">' + status + '</span>' +
            '</div>' +
            '<div class="crm-detail-sub">' + (d.cstm ? d.cstm.kontakt_typ : 'berater') + (d.cstm && d.cstm.gulp_id ? ' · Gulp ' + d.cstm.gulp_id : '') + (d.address && d.address.city ? ' · ' + d.address.city : '') + '</div>' +
            (d.title ? '<div class="crm-detail-sub">' + d.title + '</div>' : '') +
            '</div>' +
            '</div>' +
            '<div class="crm-detail-actions">' +
            '<button class="crm-action-btn crm-action-btn-primary" onclick="CRM_Berater.call((CRM_Berater._data&&CRM_Berater._data.phones&&CRM_Berater._data.phones.length?(CRM_Berater._data.phones.find(function(p){return p.field_name===\'phone_mobile\'})||CRM_Berater._data.phones[0]).raw:\'\'))"><i class="bi bi-telephone"></i> Anrufen</button>' +
            '<button class="crm-action-btn crm-action-btn-secondary" onclick="CRM_Berater.email(\'' + primaryEmail + '\')"><i class="bi bi-envelope"></i> E-Mail</button>' +
            '<button class="crm-action-btn crm-action-btn-secondary" onclick="crmEmailPopup(\'' + primaryEmail + '\',\'' + (d.first_name||'') + ' ' + (d.last_name||'') + '\',\'' + crm_id + '\')" title="E-Mail aus Vorlage"><i class="bi bi-envelope-paper"></i> Vorlage</button>' +
            '<button class="crm-action-btn crm-action-btn-secondary" onclick="CRM_Berater.showCV(\'' + crm_id + '\')"><i class="bi bi-file-text"></i> CV</button>' +
            whatsappBtn +
            '<button class="crm-action-btn" onclick="CRM_Berater.confirmDelete(\'' + crm_id + '\')" style="margin-left:auto;background:none;border:1px solid rgba(248,113,113,.5);color:#f87171;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:11px" title="Berater löschen"><i class="bi bi-trash"></i></button>' +
            '</div>' +
            '</div>' +

            '<div class="crm-detail-tabs">' +
            '<div class="crm-detail-tab active" onclick="CRM_Berater.switchTab(\'stammdaten\',this)">Stammdaten</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Berater.switchTab(\'profil\',this)">Profil</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Berater.switchTab(\'notizen\',this)">Notizen</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Berater.switchTab(\'dokumente\',this)">Dokumente</div>' +
            '</div>' +
    ...
    renderStammdaten(d) {
        ...
        (d.emails || []).forEach(function(e) {
            ...
            const typBadge = e.typ === 'privat'
                ? '<span style="font-size:9px;padding:1px 5px;border-radius:10px;background:#f3e8ff;color:#7c3aed;flex-shrink:0">Privat</span>'
                : '<span style="font-size:9px;padding:1px 5px;border-radius:10px;background:#dcfce7;color:#15803d;flex-shrink:0">Geschäftl.</span>';
            const primaerBadge = e.primary
                ? '<span style="font-size:9px;padding:1px 5px;border-radius:10px;background:#dbeafe;color:#1d4ed8;flex-shrink:0">Primär</span>'
                : '';
            const gesperrtBadge = gesperrt
                ? '<span style="font-size:9px;padding:1px 5px;border-radius:10px;background:#fee2e2;color:#dc2626;flex-shrink:0">Gesperrt</span>'
                : '';
            const kampagneChecked = e.kampagne_ok ? 'checked' : '';
            const kampagneTitle = e.kampagne_ok ? 'Kampagne erlaubt – klicken zum Deaktivieren' : 'Kampagne gesperrt – klicken zum Aktivieren';
            ...
            const kampagneHtml = gesperrt ? '' :
                ...
                '<span style="font-size:9px;color:' + kampagneColor + ';white-space:nowrap">Kamp.</span>' +
                '</label>';
            const gesperrtChecked = (e.opt_out || e.invalid_email) ? 'checked' : '';
            const gesperrtColor = (e.opt_out || e.invalid_email) ? '#dc2626' : '#9ca3af';
            const gesperrtHtml =
                '<label title="E-Mail sperren/entsperren" style="display:flex;align-items:center;gap:2px;cursor:pointer;flex-shrink:0;margin-left:2px">' +
                ...
                '<span style="font-size:9px;color:' + gesperrtColor + ';white-space:nowrap">Gesperrt</span>' +
                '</label>';
                '</label>';
            const deleteBtn = '<button onclick="CRM_Berater._deleteEmail(\'' + crm_id + '\',\'' + e.email + '\')" title="E-Mail löschen" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);padding:0 3px;font-size:11px"><i class="bi bi-trash3"></i></button>';
            const primaryBtn = !e.primary && !gesperrt
                ? '<button onclick="CRM_Berater._setPrimary(\'' + crm_id + '\',\'' + e.email + '\')" title="Als Primär setzen" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);padding:0 3px;font-size:11px"><i class="bi bi-star"></i></button>'
                : '';
            emailsHtml += '<div class="crm-info-row" style="' + rowStyle + '">' +
                (gesperrt ? '<span style="width:12px;flex-shrink:0"></span>' : CRM_Edit.renderCheckbox('email_' + e.email, e.email, 'E-Mail')) +
                '<i class="bi bi-envelope" style="color:var(--abcona-blue);font-size:12px;width:14px;flex-shrink:0"></i>' +
                '<span style="font-size:10px;color:var(--abcona-blue);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;' + textStyle + '" onclick="CRM_Berater.email(\'' + e.email + '\')" title="E-Mail öffnen">' + e.email + '</span>' +
        ...
        const leftHtml =
            E.section('Person',
                row('bi-person-badge', 'Vorname', d.first_name, 'first_name') +
                row('bi-person-badge', 'Nachname', d.last_name, 'last_name') +
                row('bi-briefcase', 'Funktion', d.title, 'title') +
                row('bi-building', 'Abteilung', d.department, 'department') +
                row('bi-cake', 'Geburtstag', d.birthdate, 'birthdate', 'date') +
                (d.account
                    ? '<div class="crm-info-row"><span style="width:12px;flex-shrink:0"></span><i class="bi bi-building" style="color:var(--abcona-blue);font-size:12px;width:14px;flex-shrink:0"></i><span style="font-size:10px;color:var(--text-muted);min-width:65px;flex-shrink:0">Firma</span><span style="font-size:11px;color:var(--abcona-blue);flex:1;cursor:pointer" onclick="window.location.href=\'/crm/kunden/?detail=' + d.account.crm_id + '\'" title="Kunden-Detail öffnen">' + d.account.name + ' <i class=\'bi bi-box-arrow-up-right\' style=\'font-size:9px;opacity:.6\'></i></span><button onclick="CRM_Berater.unlinkAccount(\'' + d.crm_id + '\')" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);font-size:11px;padding:0 3px" title="Verknüpfung lösen"><i class="bi bi-x-circle"></i></button></div>'
                    : '<div class="crm-info-row"><span style="width:12px;flex-shrink:0"></span><i class="bi bi-building" style="color:var(--abcona-blue);font-size:12px;width:14px;flex-shrink:0"></i><span style="font-size:10px;color:var(--text-muted);min-width:65px;flex-shrink:0">Firma</span><span style="font-size:11px;color:var(--text-muted);font-style:italic;flex:1">— nicht verknüpft</span><button onclick="CRM_Berater.searchAccount(\'' + d.crm_id + '\')" style="background:none;border:1px dashed var(--border-color);border-radius:4px;cursor:pointer;color:var(--abcona-blue);font-size:10px;padding:1px 6px"><i class="bi bi-search"></i> Firma</button></div>') +
                ...
                '<span style="font-size:10px;color:var(--text-muted);min-width:65px;flex-shrink:0">Nicht anrufen</span>' +
            ...
            E.section('CRM Info',
                row('bi-tag', 'Typ', c.kontakt_typ, 'kontakt_typ_c', 'select', [{value:'berater',label:'Berater'},{value:'kunde',label:'Kunde'},{value:'interessent',label:'Interessent'},{value:'andere',label:'Andere'}]) +
                row('bi-circle', 'Status', c.kontakt_status, 'kontakt_status_c', 'select', [{value:'aktiv',label:'Aktiv'},{value:'passiv',label:'Passiv'},{value:'unbekannt',label:'Unbekannt'}]) +
                row('bi-calendar-check', 'Verfügbar', c.verfuegbar_ab, 'verfuegbar_ab_c', 'date') +
                row('bi-currency-euro', 'Konditionen', c.konditionen, 'konditionen_c') +
                row('bi-star', 'Schwerpunkt', c.skill_priority, 'skill_priority_c') +
                row('bi-hash', 'Gulp-ID', c.gulp_id, 'gulp_id_c') +
                row('bi-geo-alt', 'Einsatz Stadt', c.einsatzort_stadt, 'einsatzort_stadt_c') +
                row('bi-map', 'Einsatz Region', c.einsatzort_region, 'einsatzort_region_c') +
                row('bi-signpost', 'Einsatz PLZ', c.einsatzort_plz, 'einsatzort_plz_c') +
                roRow('bi-calendar-stats', 'Gulp upd.', c.gulp_updated) +
                CRM_Edit.renderCrmInfoAdd(crm_id, c, d) +
                roRow('bi-calendar-stats', 'Xing upd.', c.xing_updated) +
                roRow('bi-calendar-stats', 'FM upd.', c.fm_updated) +
                roRow('bi-clock', 'Erstellt', d.crm_date_entered) +
                roRow('bi-clock-history', 'Geändert', d.crm_date_modified),
            false) +

            E.section('Adressen',
                '<div style="font-size:10px;font-weight:600;color:var(--abcona-blue);margin-bottom:4px"><i class="bi bi-house"></i> Hauptadresse</div>' +
                (adrHauptStr
                    ? '<div class="crm-info-row">' +
                      CRM_Edit.renderCheckbox('adr_main', adrHauptStr, 'Adresse') +
                      ...
                      '</div>'
                    : '<div style="font-size:10px;color:var(--text-muted);font-style:italic">— leer <button onclick="CRM_Berater.editAdresse(\'' + crm_id + '\',\'primary\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:11px"><i class="bi bi-plus"></i> Hinzufügen</button></div>') +
                '<div style="font-size:10px;font-weight:600;color:var(--abcona-blue);margin:8px 0 4px"><i class="bi bi-map"></i> Weitere Adresse</div>' +
                (adrAltStr
                    ? '<div class="crm-info-row">' +
                      CRM_Edit.renderCheckbox('adr_alt', adrAltStr, 'Weitere Adresse') +
                      ...
                      '</div>'
                    : '<div style="font-size:10px;color:var(--text-muted);font-style:italic">— leer <button onclick="CRM_Berater.editAdresse(\'' + crm_id + '\',\'alt\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:11px"><i class="bi bi-plus"></i> Hinzufügen</button></div>'),
            false);
        ...
        const rightHtml =
            E.section('Telefon',
                (function() {
                    const FIELD_META = {
                        phone_work:   {icon:'bi-telephone', label:'Büro'},
                        phone_mobile: {icon:'bi-phone',     label:'Mobil'},
                        phone_home:   {icon:'bi-house',     label:'Privat'},
                        phone_other:  {icon:'bi-telephone', label:'Weiteres'},
                        phone_fax:    {icon:'bi-printer',   label:'Fax'},
                    };
                    let html = '';
                    const phones = d.phones || [];
                    phones.forEach(function(p) {
                        const meta = FIELD_META[p.field_name] || {icon:'bi-telephone', label:p.field_name};
                        html += '<div class="crm-info-row">' +
                            E.renderCheckbox('ph_' + p.id, p.raw, meta.label) +
                            '<i class="bi ' + meta.icon + '" style="color:var(--abcona-blue);font-size:12px;width:14px;flex-shrink:0"></i>' +
                            '<span style="font-size:10px;color:var(--text-muted);min-width:65px;flex-shrink:0">' + (p.label || meta.label) + '</span>' +
                            '<span style="font-size:11px;flex:1;cursor:pointer" onclick="CRM_Berater.call(\'' + p.raw + '\')">' + p.raw + '</span>' +
                            '<button data-copy="' + p.raw + '" onclick="CRM_Edit.copyText(this)" title="Kopieren" style="background:none;border:none;cursor:pointer;color:var(--text-muted);padding:0 3px;font-size:11px"><i class="bi bi-clipboard"></i></button>' +
                            '<button onclick="CRM_Berater._deletePhone(\'' + crm_id + '\',' + p.id + ')" title="Löschen" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);padding:0 3px;font-size:11px"><i class="bi bi-trash3"></i></button>' +
                            '</div>';
                    });
                    // Hinzufügen
                    ...
                    html += '<div id="' + formId + '" style="display:none;gap:4px;flex-wrap:wrap;padding:4px 0;align-items:center" class="crm-info-row">' +
                        ...
                        '<option value="phone_mobile">Mobil</option>' +
                        '<option value="phone_work">Büro</option>' +
                        '<option value="phone_home">Privat</option>' +
                        '<option value="phone_other">Weiteres</option>' +
                        '<option value="phone_fax">Fax</option>' +
                        '</select>' +
                        ...
                        '<input id="ph_add_lbl_' + crm_id + '" type="text" placeholder="Bezeichnung (opt.)" style="flex:1;font-size:11px;padding:3px;border:1px solid var(--border-color);border-radius:5px;min-width:80px">' +
                        ...
                        '</div>' +
                        '<button onclick="document.getElementById(\'' + formId + '\').style.display=\'flex\'" style="background:none;border:1px dashed var(--border-color);border-radius:5px;padding:2px 8px;font-size:10px;color:var(--abcona-blue);cursor:pointer;margin-top:3px"><i class="bi bi-plus"></i> Telefon hinzufügen</button>';
                    return html;
                })(),
            true) +

            (d.assistant ? E.section('Assistent',
                row('bi-person-check', 'Name', d.assistant, 'assistant') +
                row('bi-telephone', 'Telefon', d.assistant_phone, 'assistant_phone'),
            false) : '') +

            E.section('E-Mail', emailsHtml, true) +

            E.section('Web-Profile', webHtml, true) +

            E.section('Messenger', imHtml, true);
    ...
    editAdresse(crm_id, typ) {
        ...
        const title = typ === 'primary' ? 'Hauptadresse bearbeiten' : 'Weitere Adresse bearbeiten';
        form.innerHTML = '<div style="font-size:13px;font-weight:600;color:var(--abcona-blue);margin-bottom:12px">' + title + '</div>' +
            ['street:Straße', 'city:Stadt', 'postalcode:PLZ', 'state:Bundesland', 'country:Land'].map(function(pair) {
                const parts = pair.split(':');
                const f = parts[0]; const l = parts[1];
                const v = existing[f] || '';
            ...
            '<button onclick="CRM_Berater.saveAdresse(\'' + crm_id + '\',\'' + prefix + '\')" style="background:var(--status-green);color:#fff;border:none;border-radius:5px;padding:5px 14px;cursor:pointer;font-size:12px"><i class="bi bi-check-lg"></i> Speichern</button>' +
            '<button onclick="document.getElementById(\'adr-edit-form\').remove()" style="background:var(--abcona-gray-card);border:1px solid var(--border-color);border-radius:5px;padding:5px 10px;cursor:pointer;font-size:12px"><i class="bi bi-x"></i> Abbrechen</button>' +
            '</div>';
    ...
    renderProfil(d) {
        ...
        const crm_id = d.crm_id;
        const PROFIL_MAP = [
            {key: 'ogo_description', label: 'OGO / Abcona', field: 'ogo_description_c'},
            {key: 'gulp_profil',     label: 'Gulp',          field: 'gulp_profil_c'},
            {key: 'freelancermap',   label: 'Freelancermap', field: 'freelancermap_profil_c'},
            {key: 'xing',            label: 'Xing',          field: 'xing_profile_c'},
        ];
        ...
        if (!filled.length && !empty.length) {
            return '<div class="crm-list-loading">Keine Profile vorhanden</div>';
        }
        ...
        const addBtn = empty.length
            ? '<div style="margin-left:auto;display:flex;align-items:center;gap:4px">' +
              ...
              '<option value="">+ Hinzufügen</option>' + addOpts + '</select>' +
        ...
        filled.forEach(function(p, i) {
            bodies += '<div class="crm-profil-body" data-key="' + p.key + '" id="profil-body-' + p.key + '" style="display:' + (i === 0 ? 'block' : 'none') + '">' +
                '<button onclick="CRM_Edit.startProfileEdit(\'' + crm_id + '\',\'' + p.field + '\',\'' + p.label + '\')" style="font-size:11px;border:1px solid var(--border-color);border-radius:4px;padding:2px 8px;background:none;cursor:pointer;color:var(--abcona-blue);margin-bottom:6px"><i class="bi bi-pencil"></i> Bearbeiten</button>' +
        ...
        if (!filled.length) {
            bodies = '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:11px;font-style:italic">Profil auswählen um es hinzuzufügen</div>';
        }
    ...
    renderNotizen(d) {
        ...
        const notes = (d.notes || []).map(function(n) {
            ...
            return '<div class="crm-hist-item">' +
                ...
                '</div></div>';
        }).join('') || '<div style="font-size:11px;color:var(--text-muted);padding:8px 0;font-style:italic">Noch keine Notizen</div>';

        return '<div class="crm-section"><div class="crm-section-label">Neue Notiz</div>' +
            '<textarea class="crm-note-area" id="note-text" placeholder="Notiz eingeben..."></textarea>' +
            ...
            '<option value="email">E-Mail Notiz</option>' +
            '<option value="meeting">Besprechung</option>' +
            '<option value="general">Allgemein</option>' +
            '</select>' +
            '<button class="crm-save-btn" onclick="CRM_Berater.saveNote(\'' + crm_id + '\')"><i class="bi bi-save"></i> Notiz speichern</button>' +
            '</div>' +
            '<div class="crm-section"><div class="crm-section-label">Verlauf</div>' + notes + '</div>';
    },
    ...
    renderDokumente(d) {
        const docs = (d.documents || []).map(function(doc) {
            return '<div class="crm-hist-item">' +
                ...
                '<div style="flex:1;min-width:0">' +
                '<div style="font-size:11px;font-weight:600">' + (doc.title || '') + '</div>' +
                ...
                '</div>';
        }).join('') || '<div style="font-size:11px;color:var(--text-muted);padding:8px 0;font-style:italic">Keine Dokumente</div>';
    ...
    // ── Aktionen ──────────────────────────────────────────
    call(phone) {
        if (!phone) return alert('Keine Telefonnummer hinterlegt');
    ...
    email(addr) {
        if (!addr) return alert('Keine E-Mail hinterlegt');
