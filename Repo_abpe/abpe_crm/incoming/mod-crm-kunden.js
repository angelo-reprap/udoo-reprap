...
const CRM_Kunden = {
    ...
    renderDetail(d) {
        ...
        panel.innerHTML =
            ...
            '</div>' +
            '<div class="crm-detail-sub">' + (d.address && d.address.city ? d.address.city : '') + (d.cstm && d.cstm.kunden_nr ? ' · Kd-Nr. ' + d.cstm.kunden_nr : '') + '</div>' +
            (d.industry ? '<div class="crm-detail-sub">' + d.industry + '</div>' : '') +
            '</div>' +
            '</div>' +
            '<div class="crm-detail-actions">' +
            '<button class="crm-action-btn crm-action-btn-primary" onclick="CRM_Kunden.call(\'' + firstPhone + '\')"><i class="bi bi-telephone"></i> Anrufen</button>' +
            '<button class="crm-action-btn crm-action-btn-secondary" onclick="CRM_Kunden.email(\'' + ((d.emails && d.emails.find(function(e){return e.primary;})) ? d.emails.find(function(e){return e.primary;}).email : (d.emails && d.emails[0] ? d.emails[0].email : '')) + '\')"><i class="bi bi-envelope"></i> E-Mail</button>' +
            '<button class="crm-action-btn crm-action-btn-secondary" onclick="crmEmailPopupKunde(CRM_Kunden._data)" title="E-Mail aus Vorlage"><i class="bi bi-envelope-paper"></i> Vorlage</button>' +
            (d.website ? '<button class="crm-action-btn crm-action-btn-secondary" onclick="window.open(\'' + d.website + '\',\'_blank\')"><i class="bi bi-globe"></i> Web</button>' : '') +
            '<button class="crm-action-btn crm-action-btn-secondary" onclick="CRM_Kunden.newAnfrage(\'' + crm_id + '\')"><i class="bi bi-plus"></i> Anfrage</button>' +
            '<button class="crm-action-btn" onclick="CRM_Kunden.confirmDelete(\'' + crm_id + '\')" style="margin-left:auto;background:none;border:1px solid rgba(248,113,113,.5);color:#f87171;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:11px" title="Kunde l\u00f6schen"><i class="bi bi-trash"></i></button>' +
            '</div>' +
            '</div>' +
            '<div class="crm-detail-tabs">' +
            '<div class="crm-detail-tab active" onclick="CRM_Kunden.switchTab(\'stammdaten\',this)">Stammdaten</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Kunden.switchTab(\'ansprechpartner\',this)">Ansprechpartner</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Kunden.switchTab(\'anfragen\',this)">Anfragen</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Kunden.switchTab(\'notizen\',this)">Notizen</div>' +
            '<div class="crm-detail-tab" onclick="CRM_Kunden.switchTab(\'dokumente\',this)">Dokumente</div>' +
            '</div>' +
    ...
    renderStammdaten(d) {
        ...
        (d.emails || []).forEach(function(e) {
            ...
            const primaerBadge = e.primary ? '<span style="font-size:9px;padding:1px 5px;border-radius:10px;background:#dbeafe;color:#1d4ed8;flex-shrink:0">Prim\u00e4r</span>' : '';
            const gesperrtBadge = gesperrt ? '<span style="font-size:9px;padding:1px 5px;border-radius:10px;background:#fee2e2;color:#dc2626;flex-shrink:0">Gesperrt</span>' : '';
            ...
            const kampagneHtml2 = gesperrt ? '' :
                ...
                '<span style="font-size:9px;color:' + kampagneColor2 + ';white-space:nowrap">Kamp.</span></label>';
            ...
            const gesperrtHtml2 =
                ...
                '<span style="font-size:9px;color:' + gesperrtColor2 + ';white-space:nowrap">Gesperrt</span></label>';
            const deleteBtn2 = '<button onclick="CRM_Kunden._deleteEmail(\'' + crm_id + '\',\'' + e.email + '\')" title="L\u00f6schen" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);padding:0 3px;font-size:11px"><i class="bi bi-trash3"></i></button>';
            const primaryBtn2 = !e.primary && !gesperrt ? '<button onclick="CRM_Kunden._setPrimary(\'' + crm_id + '\',\'' + e.email + '\')" title="Als Prim\u00e4r setzen" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);padding:0 3px;font-size:11px"><i class="bi bi-star"></i></button>' : '';
            emailsHtml += '<div class="crm-info-row" style="' + rowStyle + '">' +
                (gesperrt ? '<span style="width:12px;flex-shrink:0"></span>' : E.renderCheckbox('email_' + e.email, e.email, 'E-Mail')) +
                '<i class="bi bi-envelope" style="color:var(--abcona-blue);font-size:12px;width:14px;flex-shrink:0"></i>' +
                '<span style="font-size:10px;color:var(--abcona-blue);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;' + textStyle + '" onclick="CRM_Kunden.email(\'' + e.email + '\')" title="E-Mail \u00f6ffnen">' + e.email + '</span>' +
        ...
        const leftHtml =
            E.section('Firma',
                row('bi-building', 'Name', d.name, 'name') +
                row('bi-tag', 'Typ', d.account_type, 'account_type', 'select', [
                    {value:'Analyst',label:'Analyst'},{value:'Competitor',label:'Wettbewerber'},
                    {value:'Customer',label:'Kunde'},{value:'Integrator',label:'Integrator'},
                    {value:'Investor',label:'Investor'},{value:'Partner',label:'Partner'},
                    {value:'Press',label:'Presse'},{value:'Prospect',label:'Interessent'},
                    {value:'Reseller',label:'Reseller'},{value:'Other',label:'Sonstiges'},
                ]) +
                row('bi-briefcase', 'Branche', d.industry, 'industry') +
                row('bi-globe', 'Website', d.website, 'website') +
                row('bi-people', 'Mitarbeiter', d.employees, 'employees') +
                row('bi-currency-euro', 'Jahresumsatz', d.annual_revenue, 'annual_revenue') +
                row('bi-star', 'Einstufung', d.rating, 'rating') +
                (d.description ? '<div class="crm-info-row" style="align-items:flex-start"><i class="bi bi-text-paragraph" style="color:var(--abcona-blue);font-size:12px;width:14px;flex-shrink:0;margin-top:2px"></i><span style="font-size:10px;color:var(--text-muted);min-width:70px;flex-shrink:0">Beschreibung</span>' + E.editField(crm_id, 'description', d.description, 'textarea') + '</div>' : ''),
            true) +
            E.section('CRM Info',
                row('bi-circle', 'Status', c.status, 'account_status_c', 'select', [
                    {value:'aktiv',label:'Aktiv'},{value:'passiv',label:'Passiv'},{value:'unbekannt',label:'Unbekannt'},
                ]) +
                row('bi-hash', 'Kd-Nummer', c.kunden_nr, 'kunden_nummer_c') +
                roRow('bi-clock', 'Erstellt', d.crm_date_entered) +
                roRow('bi-clock-history', 'Ge\u00e4ndert', d.crm_date_modified),
            false) +
            E.section('Adressen',
                '<div style="font-size:10px;font-weight:600;color:var(--abcona-blue);margin-bottom:4px"><i class="bi bi-receipt"></i> Rechnungsadresse</div>' +
                (bilStr ? '<div class="crm-info-row">' + E.renderCheckbox('adr_billing', bilStr, 'Rechnungsadresse') + '<span style="font-size:11px;flex:1">' + bilStr + '</span><button onclick="CRM_Kunden.editAdresse(\'' + crm_id + '\',\'billing\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:11px"><i class="bi bi-pencil"></i></button><button onclick="CRM_Kunden._clearAdresse(\'' + crm_id + '\',\'billing\')" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);font-size:11px"><i class="bi bi-trash3"></i></button></div>' : '<div style="font-size:10px;color:var(--text-muted);font-style:italic">\u2014 leer <button onclick="CRM_Kunden.editAdresse(\'' + crm_id + '\',\'billing\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:11px"><i class="bi bi-plus"></i> Hinzuf\u00fcgen</button></div>') +
                '<div style="font-size:10px;font-weight:600;color:var(--abcona-blue);margin:8px 0 4px"><i class="bi bi-truck"></i> Lieferadresse</div>' +
                (shiStr ? '<div class="crm-info-row">' + E.renderCheckbox('adr_shipping', shiStr, 'Lieferadresse') + '<span style="font-size:11px;flex:1">' + shiStr + '</span><button onclick="CRM_Kunden.editAdresse(\'' + crm_id + '\',\'shipping\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:11px"><i class="bi bi-pencil"></i></button><button onclick="CRM_Kunden._clearAdresse(\'' + crm_id + '\',\'shipping\')" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);font-size:11px"><i class="bi bi-trash3"></i></button></div>' : '<div style="font-size:10px;color:var(--text-muted);font-style:italic">\u2014 leer <button onclick="CRM_Kunden.editAdresse(\'' + crm_id + '\',\'shipping\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:11px"><i class="bi bi-plus"></i> Hinzuf\u00fcgen</button>' + (bilStr ? ' <button onclick="CRM_Kunden.copyBillingToShipping(\'' + crm_id + '\')" style="background:none;border:none;cursor:pointer;color:var(--abcona-blue);font-size:10px"><i class="bi bi-copy"></i> Von Rechnungsadresse</button>' : '') + '</div>'),
            false);
        ...
        const rightHtml =
            E.section('Telefon',
                (function() {
                    const FIELD_META = {
                        phone_office:    {icon:'bi-telephone', label:'B\u00fcro'},
                        phone_alternate: {icon:'bi-telephone', label:'Alternativ'},
                    ...
                    phones.forEach(function(p) {
                        ...
                        html += '<div class="crm-info-row">' +
                            ...
                            '<button onclick="CRM_Kunden._deletePhone(\'' + crm_id + '\',' + p.id + ')" title="L\u00f6schen" style="background:none;border:none;cursor:pointer;color:var(--badge-error-text);padding:0 3px;font-size:11px"><i class="bi bi-trash3"></i></button>' +
                            '</div>';
                    });
                    ...
                    html += '<div id="' + formId + '" style="display:none;gap:4px;flex-wrap:wrap;padding:4px 0;align-items:center" class="crm-info-row">' +
                        ...
                        '<option value="phone_office">B\u00fcro</option>' +
                        '<option value="phone_alternate">Alternativ</option>' +
    ...
    editAdresse(crm_id, typ) {
        ...
        const title = typ === 'billing' ? 'Rechnungsadresse bearbeiten' : 'Lieferadresse bearbeiten';
        ...
        form.innerHTML = '<div style="font-size:13px;font-weight:600;color:var(--abcona-blue-light,#6fa3d8);margin-bottom:12px">' + title + '</div>' +
            ['street:Stra\u00dfe', 'city:Stadt', 'postalcode:PLZ', 'state:Bundesland', 'country:Land'].map(function(pair) {
                const parts = pair.split(':'); const f = parts[0]; const l = parts[1]; const v = existing[f] || '';
    ...
    renderAnsprechpartner(d) {
        ...
        const addBtn = '<div style="display:flex;justify-content:flex-end;margin-bottom:8px"><button onclick="CRM_Kunden.newAnsprechpartner(\'' + accountId + '\')" style="padding:5px 12px;font-size:11px;background:var(--abcona-blue);color:#fff;border:none;border-radius:6px;cursor:pointer"><i class="bi bi-person-plus"></i> Neuer Ansprechpartner</button></div>';
        if (!ap.length) return addBtn + '<div class="crm-list-loading"><i class="bi bi-people"></i> Keine Ansprechpartner</div>';
        return addBtn + ap.map(function(a) {
            ...
            const emailHtml = emails.map(function(e) {
                return '<div style="font-size:11px;margin-top:2px"><i class="bi bi-envelope" style="color:var(--abcona-blue);font-size:11px"></i> <span style="color:var(--abcona-blue);cursor:pointer" onclick="CRM_Kunden.email(\'' + e.email.replace(/'/g, "\\'") + '\')">' + e.email + '</span>' + (e.primary ? ' <span style="font-size:9px;padding:1px 4px;border-radius:8px;background:#dbeafe;color:#1d4ed8">Prim\u00e4r</span>' : '') + '</div>';
            }).join('');
            return '<div class="crm-hist-item" style="align-items:flex-start;gap:10px;padding:8px 0">' +
                ...
                (phones.length ? '<button class="crm-action-btn crm-action-btn-secondary" style="padding:3px 8px;font-size:10px" onclick="CRM_Kunden.call(\'' + phones[0].raw + '\')" title="Anrufen"><i class="bi bi-telephone"></i></button>' : '') +
                (primEmail ? '<button class="crm-action-btn crm-action-btn-secondary" style="padding:3px 8px;font-size:10px" onclick="CRM_Kunden.email(\'' + primEmail.email.replace(/'/g, "\\'") + '\')" title="E-Mail"><i class="bi bi-envelope"></i></button>' : '') +
                (contactId ? '<button class="crm-action-btn crm-action-btn-secondary" style="padding:3px 8px;font-size:10px" onclick="CRM_Kunden._openContact(\'' + contactId + '\')" title="Kontakt \u00f6ffnen"><i class="bi bi-pencil"></i></button>' : '') +
                '</div></div>';
        }).join('');
    },
    ...
    renderAnfragen(d) {
        ...
        if (!anf.length) return '<div class="crm-list-loading"><i class="bi bi-inbox"></i> Keine Anfragen</div>';
    ...
    renderNotizen(d) {
        ...
        return '<div class="crm-section"><div class="crm-section-label">Neue Notiz</div><textarea class="crm-note-area" id="note-text" placeholder="Notiz eingeben..."></textarea><select style="width:100%;margin-top:6px;padding:5px;border:1px solid var(--border-color);border-radius:7px;font-size:12px" id="note-type"><option value="phone">Telefonnotiz</option><option value="email">E-Mail Notiz</option><option value="meeting">Besprechung</option><option value="general">Allgemein</option></select><button class="crm-save-btn" onclick="CRM_Kunden.saveNote(\'' + crm_id + '\')"><i class="bi bi-save"></i> Notiz speichern</button></div><div class="crm-section"><div class="crm-section-label">Verlauf</div>' + notes + '</div>';
    },

    renderDokumente(d) {
        const docs = (d.documents || []).map(function(doc) {
            return '<div class="crm-hist-item"><i class="bi bi-file-text" style="color:var(--abcona-blue)"></i><div style="flex:1;min-width:0"><div style="font-size:11px;font-weight:600">' + (doc.title || '') + '</div><div style="font-size:10px;color:var(--text-muted)">' + (doc.doc_type || '') + ' \u00b7 ' + (doc.created_at || '').substring(0, 10) + '</div></div>' + (doc.file_path ? '<a href="' + doc.file_path + '" target="_blank" style="font-size:10px;color:var(--abcona-blue)"><i class="bi bi-download"></i></a>' : '') + '</div>';
    ...
    newAnfrage(crm_id) {
        window.location.href = '/matching/?new=1&crm_account_id=' + crm_id;
    },
