...
import json
from apps.abpe_crm.core.module_scanner import scanner
...
def available_languages(request):
    ...
    i18n_dir = Path(__file__).parent / 'static' / 'abpe_crm' / 'i18n'
    languages = []
    if i18n_dir.exists():
...
def navigation(request):
    ...
    if user and user.is_authenticated:
        try:
            from apps.abpe_crm.models import UserSettings
    ...
    return {
        'crm_navigation': main_navigation,
        'crm_system_navigation': system_navigation,
