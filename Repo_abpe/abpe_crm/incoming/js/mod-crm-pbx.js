...
Object.assign(PBX, {
    ...
    async _mmWizardSend() {
        ...
        this.meetmeCloseModal();
        this.toast(this._mmNotifySuccessToast());
        await this.meetmeLoadMeetings();
    },
...
Object.assign(PBX, {
    ...
    async _mmNotifyPrepareState(meetingId, action) {
        ...
        st.meetingId = meetingId;
        st.action = action;
        st.mode = action === 'invite' ? 'all' : 'individual';
        st.templates = (tplData && tplData.templates) || [];
    ...
    async _mmNotifyCommit() {
        ...
        try {
            ...
            } else if (st.action === 'reschedule') {
                await this.post(`/meetme/api/meetings/${st.meetingId}/reschedule/`, { new_start_at: st.newStartAt });
            }
            // invite: kein reschedule — Meeting existiert bereits, nur notify-bulk/send-adhoc
            return true;
        } catch (e) {
    ...
    _mmNotifySuccessToast() {
        const st = this._mmNotifyState;
        if (st.action === 'cancel') return this.t('pbx_meetme_cancelled', 'Termin abgesagt');
        if (st.action === 'invite') return this.t('pbx_mm_invite_sent', 'Einladungen versendet');
        return this.t('pbx_meetme_saved', 'Gespeichert');
    },
