...
const CRM = {
    ...
    renderList(results) {
        ...
        if (!results.length) {
            const msg = window.CRM_I18N ? window.CRM_I18N.t('keine_treffer', 'Keine Treffer') : 'Keine Treffer';
            list.innerHTML = '<div class="crm-list-loading">' + msg + '</div>';
            return;
        }
    ...
    renderBeraterItem(r) {
        ...
        <div class="crm-list-item" data-crm-id="${r.crm_id}">
            ...
            <div class="crm-item-right">
                <span class="crm-badge ${badgeCls}">${window.CRM_I18N ? window.CRM_I18N.status(status) : status}</span>
    ...
    loadStats() {
        fetch('/crm/api/sync/status/', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(data => {
                ...
                const sectionCount = document.getElementById('crm-section-count');
                if (sectionCount && vals[0]) sectionCount.textContent = '(' + vals[0].toLocaleString() + ')';
            })
            .catch(() => {});
    },
    ...
    renderPagination(total, pages, current) {
        ...
        const n = (window.CRM_I18N ? window.CRM_I18N.t('eintraege', 'Einträge') : 'Einträge');
        html += `<span style="font-size:11px;color:var(--text-muted);margin-left:8px">${total} ${n}</span>`;
        el.innerHTML = html;
        this._lastPagination = { total, pages, current };
    },
...
// ── i18n Helpers (Detail-Panels, Listen-Badges) ───────────
window.CRM_I18N = {
    t(key, fallback) {
    ...
    status(code) {
        if (!code) return '';
        return this.t(code, code);
    },
    statusOpts() {
        return [
            { value: 'aktiv', label: this.t('aktiv', 'Aktiv') },
            { value: 'passiv', label: this.t('passiv', 'Passiv') },
            { value: 'unbekannt', label: this.t('unbekannt', 'Unbekannt') },
        ];
    },
    kontaktTypOpts() {
        return [
            { value: 'berater', label: this.t('berater', 'Berater') },
            { value: 'kunde', label: this.t('kunde_label', 'Kunde') },
            { value: 'interessent', label: this.t('interessent', 'Interessent') },
            { value: 'andere', label: this.t('andere', 'Andere') },
        ];
    },
    accountTypeOpts() {
        return [
            { value: 'Analyst', label: this.t('acc_analyst', 'Analyst') },
            { value: 'Competitor', label: this.t('acc_competitor', 'Wettbewerber') },
            { value: 'Customer', label: this.t('acc_customer', 'Kunde') },
            { value: 'Integrator', label: this.t('acc_integrator', 'Integrator') },
            { value: 'Investor', label: this.t('acc_investor', 'Investor') },
            { value: 'Partner', label: this.t('acc_partner', 'Partner') },
            { value: 'Press', label: this.t('acc_press', 'Presse') },
            { value: 'Prospect', label: this.t('acc_prospect', 'Interessent') },
            { value: 'Reseller', label: this.t('acc_reseller', 'Reseller') },
            { value: 'Other', label: this.t('acc_other', 'Sonstiges') },
        ];
    },
    phoneLabel(field) {
        const map = {
            phone_office: 'buero', phone_work: 'buero', phone_alternate: 'alternativ',
            phone_fax: 'fax', phone_mobile: 'mobil', phone_home: 'privat', phone_other: 'weiteres',
        };
        const key = map[field];
        return key ? this.t(key, field) : field;
    },
    refreshUi() {
        if (window.CRM_TAB === 'kunden' && window.CRM_Kunden && CRM_Kunden._data) {
            const tab = CRM_Kunden._activeTab || 'stammdaten';
            CRM_Kunden.renderDetail(CRM_Kunden._data);
            const el = document.querySelector('.crm-detail-tab[onclick*="' + tab + '"]');
            CRM_Kunden.switchTab(tab, el);
        }
        if (window.CRM_TAB === 'berater' && window.CRM_Berater && CRM_Berater._data) {
            const tab = CRM_Berater._activeTab || 'stammdaten';
            CRM_Berater.renderDetail(CRM_Berater._data);
            const el = document.querySelector('.crm-detail-tab[onclick*="' + tab + '"]');
            CRM_Berater.switchTab(tab, el);
        }
        if (window.CRM && CRM._lastResults) CRM.renderList(CRM._lastResults);
        if (window.CRM && CRM._lastPagination) {
            const p = CRM._lastPagination;
            CRM.renderPagination(p.total, p.pages, p.current);
        }
    },
};
