...
class MatchingAnfrageExtractRequestSerializer(serializers.Serializer):
    email_text = serializers.CharField(
        min_length=20,
        help_text='E-Mail-Body / Freitext der Anfrage',
    )
    subject = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text='Optional: Betreffzeile',
    )
    outer_from = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text='Optional: äußerer Absender (bei Weiterleitungen)',
    )


class MatchingAnfrageExtractResponseSerializer(serializers.Serializer):
    success = serializers.BooleanField()
    extract = serializers.JSONField(required=False)
    fields = serializers.JSONField(required=False)
    validation = serializers.JSONField(required=False)
    prompt_key = serializers.CharField(required=False, allow_blank=True)
    source = serializers.CharField(required=False, allow_blank=True)
