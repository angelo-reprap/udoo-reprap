...
class UserSettings(models.Model):
    ...
    phone_pin            = models.CharField(max_length=20, blank=True, default="", verbose_name="Asterisk PIN")
    phone_display_name   = models.CharField(max_length=50, blank=True, default="", verbose_name="Anzeigename")
    phone_enabled        = models.BooleanField(default=False, verbose_name="Telefon aktiv")
    phone_webdial_url     = models.CharField(max_length=200, blank=True, default="", verbose_name="Webdial URL")
    ...
    phone_int_prefix     = models.CharField(max_length=5, blank=True, default="00", verbose_name="Intl. Prefix (+ Ersatz)")
...
class GroupModulePermission(models.Model):
    """
    Gruppen-spezifische Modul-Berechtigungen.
    denied=True  → Gruppe darf Modul NICHT sehen
    denied=False → Gruppe darf Modul sehen (explizit erlaubt)
    Überschreibt die roles in module.json.
    """
    ...
    def __str__(self):
        status = 'gesperrt' if self.denied else 'erlaubt'
        return f"{self.group.name} → {self.module_id} ({status})"
