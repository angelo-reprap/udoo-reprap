...
def api_berater_list(request):
    ...
    if q:
        from .models import CrmEmailAddrBeanRel
        email_contact_ids = CrmEmailAddrBeanRel.objects.filter(
        ...
        from apps.abpe_crm.models import CrmPhoneBeanRel as _PhoneRel
        q_parts = q.split()
        q_norm = normalize_phone(q)
        phone_contact_ids = _PhoneRel.objects.filter(
            bean_module='Contacts',
            phone__phone_norm__icontains=q_norm if len(q_norm) >= 6 else q
        ).values_list('bean_id', flat=True) if len(_re.sub(r'\D','',q)) >= 4 else []
        qs = qs.filter(
            Q(first_name__icontains=q) |
            Q(last_name__icontains=q) |
            Q(first_name__icontains=q_parts[0]) |
            Q(last_name__icontains=q_parts[-1]) |
            Q(crm_id__in=email_contact_ids) |
            Q(crm_id__in=phone_contact_ids)
        )
