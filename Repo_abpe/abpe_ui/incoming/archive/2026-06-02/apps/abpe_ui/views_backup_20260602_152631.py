...
def api_set_language(request):
    ...
    # Nur erlaubte Sprachen akzeptieren
...
def api_user_settings(request):
    ...
    if request.method == 'POST':
        ...
        if 'nav_order' in data:
            settings.nav_order = data['nav_order']
    ...
    return JsonResponse({
        ...
        'data': {
            ...
            'nav_order': settings.nav_order or [],
        }
    })
...
def api_admin_user_toggle(request, uid):
    """POST: User aktiv/inaktiv umschalten — eigenen User nicht deaktivieren"""
    try:
...
# ============================================================
# USER MODULE PERMISSIONS API
# ============================================================

@admin_required
def api_admin_user_module_permissions(request, uid):
    """
    GET:  Modul-Berechtigungen eines Users laden
    POST: Modul-Berechtigungen eines Users setzen
    """
    ...
    if request.method == 'POST':
        ...
        # Alle bestehenden Einträge löschen und neu setzen
    ...
    # GET: aktuelle Ausschlüsse + alle verfügbaren Module
    denied = list(
        UserModulePermission.objects.filter(user=user, denied=True)
        .values_list('module_id', flat=True)
    )
    all_modules = [
        {'id': m['id'], 'title': m['title'], 'icon': m.get('icon', 'puzzle')}
...
def api_admin_group_module_permissions(request, gid):
    """
    GET:  Modul-Berechtigungen einer Gruppe laden
    POST: Modul-Berechtigungen einer Gruppe setzen
    """
    ...
    if request.method == 'POST':
        ...
        for module_id in denied_modules:
            GroupModulePermission.objects.create(
                group=group,
                module_id=module_id,
                denied=True
            )
    ...
    # GET
    denied = list(
        GroupModulePermission.objects.filter(group=group, denied=True)
        .values_list('module_id', flat=True)
    )
    all_modules = [
        {'id': m['id'], 'title': m['title'], 'icon': m.get('icon', 'puzzle')}
...
def cv_editor_view(request):
    """CV Editor — Liste aller Consultants"""
...
def api_cv_editor_consultant(request, aid):
    """
    Liefert alle Consultant-Daten für den Portal CV Editor.
    Gleiche Logik wie cv_extractor.views.editor_view.
    """
    ...
    # Sprachen
    languages = [
        f"{cl.language.name}{' (' + cl.level + ')' if cl.level else ''}"
        for cl in consultant.languages.all()
    ]

    # Ausbildung
    education = [
        {'degree': e.degree, 'institution': e.institution, 'period': e.period or '', 'type': e.education_type}
        for e in consultant.education.all()
    ]

    # Branchen, Fachbereiche, Zertifikate, Produkte
...
def api_es_search(request):
    ...
    try:
        ...
        must_not = []
        if excludes and idx_type == 'email':
            must_not = [{'term': {'account': acc}} for acc in excludes]
        ...
        # Echte Gesamtzahl per count-Query
        ...
        return JsonResponse({'total': total, 'results': results, 'query': query, 'type': idx_type})
    except Exception as e:
        return JsonResponse({'error': str(e), 'results': [], 'total': 0}, status=500)

def api_email_view(request):
    """E-Mail live von IMAP holen und als HTML zurückgeben"""
    ...
    if not all([account, folder, message_id]):
