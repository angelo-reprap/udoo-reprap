...
from .models import (
    ...
    EmailTemplateVersion, EmailLog, EmailQueue
)
...
class EmailTemplateVersionInline(admin.TabularInline):
    ...
    extra          = 0
    readonly_fields = ['version', 'sender_mode', 'change_note', 'created_by', 'created_at']
    fields         = ['version', 'sender_mode', 'change_note', 'created_by', 'created_at']
    can_delete     = False
...
class EmailTemplateAdmin(admin.ModelAdmin):
    list_display   = ['name', 'identifier', 'app_scope', 'sender_mode',
                      'status', 'active_version', 'usage_count', 'last_used_at']
    list_filter    = ['status', 'app_scope', 'sender_mode']
    ...
    fieldsets      = [
        ('Basis', {'fields': ['identifier', 'name', 'description', 'app_scope', 'event_type']}),
        ('Absender', {'fields': ['sender_mode', 'sender_account', 'signature', 'include_signature']}),
        ('CC / BCC', {'fields': ['cc_emails', 'bcc_emails']}),
        ('Inhalt', {'fields': ['subject', 'html_body', 'text_body', 'variables']}),
        ('Status', {'fields': ['status', 'active_version']}),
        ('Tracking', {'fields': ['usage_count', 'last_used_at', 'created_by', 'created_at', 'updated_at']}),
    ]
