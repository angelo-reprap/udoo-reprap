...
class DocAssembler:
    ...
    def _apply_header(self, doc: DocxDocument, layout,
                       variables: dict) -> None:
        """
        Word-Header: nur Seitennummer rechts + Trennlinie.
        Logo + Kontakt kommen als LOGO-Block im Body (2-spaltige Tabelle).
        Format der Seitennummer aus layout.page_number_format.
        Stil aus StyleDefinition 'footer_text' (StyleKit des Templates).
        """
        # StyleKit aus erstem Template das dieses Layout nutzt
        # Fallback: direkt aus DB
        from apps.abpe_doc_studio.models import StyleKit
        sk = StyleKit.objects.filter(
            identifier='abcona_standard'
        ).prefetch_related('definitions').first()

        sdef = self.style_cache.get(sk, 'footer_text') if sk else None
        font_name = sdef.font_family if sdef else FONT_NAME
        font_size = sdef.font_size_pt if sdef else 8.0
        color     = sdef.color_hex   if sdef else COLOR_BRAND

        # Format aus Layout: "Seite {page} von {total}"
        nr_fmt      = layout.page_number_format
        before_page = nr_fmt.split('{page}')[0]
        rest        = nr_fmt.split('{page}')[1] if '{page}' in nr_fmt else ''
        between     = rest.split('{total}')[0]  if '{total}' in rest  else ' von '
        ...
        for section in doc.sections:
            ...
            p_nr.paragraph_format.space_before = Pt(0)
            p_nr.paragraph_format.space_after  = Pt(2)

            # "Seite "
            if before_page:
                r = p_nr.add_run(before_page)
                r.font.name = font_name
            ...
            # nach {total}
            if after_total:
                r3 = p_nr.add_run(after_total)
                r3.font.name = font_name
                r3.font.size = Pt(font_size)
                r3.font.color.rgb = _rgb(COLOR_BRAND)

            # Trennlinie unter Seitennummer
            p_line = hdr.add_paragraph()
    ...
    def _render_logo(self, doc, variables: dict) -> None:
        """
        LOGO-Block = 2-spaltige Tabelle im Body.

        Linke Spalte (~60%): wird LEER gelassen.
        Der Assembler füllt sie NACHTRÄGLICH mit DOC_TITLE + PARTY_BLOCKs
        (see _build_docx — logo_table Mechanismus).

        Rechte Spalte (~40%): Logo-Bild + Kontaktzeilen aus block.content.
        Format block.content (abcona_logo):
          Zeile 0: "{ag_firma}|Tagline"  → logo_contact_title + logo_contact_tagline
          Zeile 1+: "Label: {var}"        → logo_contact_line

        Styles aus StyleKit:
          logo_contact_title   → bold+italic, blau, rechts
          logo_contact_tagline → italic, blau, rechts
          logo_contact_line    → normal, dunkel, rechts
        """
        import os
        ...
        # Kontaktzeilen aus ContentBlock 'abcona_logo'
        try:
            from apps.abpe_doc_studio.models import ContentBlock
            logo_block = ContentBlock.objects.filter(
                identifier='abcona_logo', is_active=True
            ).select_related('style_kit').first()
            raw_content = logo_block.content if logo_block else ''
            sk = logo_block.style_kit if logo_block else None
        except Exception as e:
            log.warning(f'_render_logo: {e}')
            raw_content = ''
            sk = None

        rendered = self.variable_engine.render_text(
            raw_content, variables or {}
        )
        contact_lines = [
            l.strip() for l in rendered.split('\n') if l.strip()
        ]

        # Geometrie aus Layout (wird über doc.sections gelesen)
        # content_width = 15cm (aus layout.json)
        # linke Spalte: 9cm, rechte Spalte: 6cm
        left_dxa  = int(9.0  * 567)   # 9cm
        right_dxa = int(6.0  * 567)   # 6cm

        # 2-spaltige Tabelle ohne Rahmen
        ...
        # Linke Spalte: leer — wird später durch _collect_left_cells befüllt
        lc.paragraphs[0].clear()

        # Rechte Spalte: Logo-Bild
        rp = rc.paragraphs[0]
        rp.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        rp.paragraph_format.space_before = Pt(0)
        rp.paragraph_format.space_after  = Pt(4)
        if logo_path:
            rp.add_run().add_picture(logo_path, height=Cm(2.0))
        else:
            r_fb = rp.add_run(contact_lines[0].split('|')[0]
                              if contact_lines else '')
            self._apply_style(rp, 'logo_contact_title', sk, r_fb)

        # Rechte Spalte: Kontaktzeilen
        # Zeile 0 → Firmenname + Tagline (pipe-separiert)
        # Zeile 1+ → normale Kontaktzeilen
        for i, line in enumerate(contact_lines):
            if '|' in line:
                ...
                if firma.strip():
                    ...
                    self._apply_style(p2, 'logo_contact_title', sk, r2)
                if tagline.strip():
                    p3 = rc.add_paragraph()
                    r3 = p3.add_run(tagline.strip())
                    self._apply_style(p3, 'logo_contact_tagline', sk, r3)
            else:
                p4 = rc.add_paragraph()
                r4 = p4.add_run(line.strip())
                self._apply_style(p4, 'logo_contact_line', sk, r4)

        # Tabellen-Referenz merken für nachfolgende Blöcke
        doc._logo_table    = tbl
        doc._logo_left_cell = lc
        doc._logo_rows_used = 0
