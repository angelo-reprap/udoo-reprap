...
class PageLayout(models.Model):
    ...
    page_number_position = models.CharField(
        max_length=20, default='top_right', blank=True
    )
    logo_column_widths_cm = models.JSONField(
        default=list, blank=True,
        verbose_name='Logo-Spaltenbreiten (cm)',
        help_text='[links_cm, rechts_cm] z.B. [9.0, 6.0]'
    )
    logo_height_cm = models.FloatField(
        default=2.0, blank=True,
        verbose_name='Logo-Hoehe (cm)'
    )
    label_column_widths_cm = models.JSONField(
        default=list, blank=True,
        verbose_name='Label-Value Spaltenbreiten (cm)'
    )
    signature_column_widths_cm = models.JSONField(
        default=list, blank=True,
        verbose_name='Signatur Spaltenbreiten (cm)'
    )
    total_column_widths_cm = models.JSONField(
        default=list, blank=True,
        verbose_name='Total-Block Spaltenbreiten (cm)'
    )
    inv_header_column_widths_cm = models.JSONField(
        default=list, blank=True,
        verbose_name='Rechnungskopf Spaltenbreiten (cm)'
    )
    table_alt_row_color = models.CharField(
        max_length=7, default='F8FAFC', blank=True,
        verbose_name='Tabelle Alt-Zeilenfarbe (hex)'
    )
    table_white_row_color = models.CharField(
        max_length=7, default='FFFFFF', blank=True,
        verbose_name='Tabelle Weiss-Zeilenfarbe (hex)'
    )
    normal_font_size_pt = models.FloatField(
        default=10.0, blank=True,
...
class DocTemplate(models.Model):
    ...
    # Shared Block-Referenzen (aus template.json)
