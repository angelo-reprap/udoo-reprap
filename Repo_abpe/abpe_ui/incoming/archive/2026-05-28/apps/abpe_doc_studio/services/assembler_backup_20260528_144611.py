...
class DocAssembler:
    ...
    def _render_paragraph(self, doc, content: str,
        ...
        for i, line in enumerate(lines):
            ...
            if '<br' in line:
                # Weicher Zeilenumbruch mit optionalem Style-Attribut.
                # Syntax: <br/> oder
                #   <br align="left"/>
                #   <br style="font-weight: bold; font-style: italic; font-size: 10pt; color: #163258; text-align: left;"/>
                # CSS-Properties:
                #   font-weight: bold/normal
                #   font-style: italic/normal
                #   text-decoration: underline/none
                #   font-size: Npt  (z.B. 10pt)
                #   color: #RRGGBB oder RRGGBB
                #   text-align: left/right/center/justify
                # Alle Teile landen in EINEM Paragraph — kein neuer Absatz.
                import re as _re
                align_map_br = {
                    'left':    WD_ALIGN_PARAGRAPH.LEFT,
                    'right':   WD_ALIGN_PARAGRAPH.RIGHT,
                ...
                def _parse_br_style(tag_str):
                    """Parst style="" und align="" aus einem <br.../> Tag."""
                    result = {}
                    # align="..."
                    m = _re.search(r'align="([^"]+)"', tag_str)
                    if m:
                        result['text-align'] = m.group(1)
                    # style="..."
                    m = _re.search(r'style="([^"]+)"', tag_str)
                    if m:
                ...
                self._apply_style(p, skey, sk)

                # Aufteilen: Text und <br.../> Tags abwechselnd
                raw_parts = _re.split(r'(<br[^/]*/?>)', line)
                # raw_parts = [text, <br.../>, text, <br.../>, ...]
                cur_style = {}  # aktuelle Style-Overrides

                for idx, segment in enumerate(raw_parts):
                    if segment.startswith('<br'):
                        # Style aus diesem <br> für nächsten Segment merken
                        cur_style = _parse_br_style(segment)
                    ...
                    if not segment.strip():
                        continue
                    # Alignment setzen
                    align_val = cur_style.get('text-align', sdef.alignment if sdef else 'left')
                    p.alignment = align_map_br.get(align_val, WD_ALIGN_PARAGRAPH.LEFT)
                    # Run erstellen
                    run = p.add_run(segment.strip())
                    if sdef:
                        run.font.name = sdef.font_family
                        run.font.size = Pt(float(cur_style.get('font-size', f'{sdef.font_size_pt}pt').replace('pt','')))
                        run.bold      = (cur_style.get('font-weight', 'bold' if sdef.bold else 'normal') == 'bold')
                        run.italic    = (cur_style.get('font-style', 'italic' if sdef.italic else 'normal') == 'italic')
                        run.underline = (cur_style.get('text-decoration', 'underline' if sdef.underline else 'none') == 'underline')
                        color_val = cur_style.get('color', sdef.color_hex).lstrip('#')
                        run.font.color.rgb = _rgb(color_val)
                    cur_style = {}  # Reset nach jedem Segment
            elif line.strip():
    ...
    def _render_table(self, doc, content: str,
        ...
        row_borders = {}
        if hasattr(block, 'row_borders') and block.row_borders:
            raw = block.row_borders
            if isinstance(raw, str):
                import json as _j
                try: raw = _j.loads(raw)
                except: raw = {}
            row_borders = {int(k): v for k, v in raw.items()}
        bb_sdef    = self.style_cache.get(sk, 'border_signature')

        col_borders = {}
        if hasattr(block, 'col_borders') and block.col_borders:
            raw = block.col_borders
            if isinstance(raw, str):
                import json as _j
                try: raw = _j.loads(raw)
                except: raw = {}
            col_borders = {int(k): v for k, v in raw.items()}
    ...
    def _render_static_table(self, doc, content: str,
                              ...
                              row_bdr: list,
                              bb_sdef,
                              row_borders: dict = None,
                              col_borders: dict = None) -> None:
        """Statische Tabelle aus 'sp1|sp2|sp3' pro Zeile."""
        if row_borders is None:
            row_borders = {}
        if col_borders is None:
            col_borders = {}
        ...
        for ri, line in enumerate(lines):
            ...
            for ci in range(n_cols):
                ...
                border_cfg = {}
                if ci in col_borders:
                    border_cfg.update(col_borders[ci])
                if ri in row_borders:
                    rb = row_borders[ri]
                    # cols: Liste von Spaltenindizes die betroffen sind
                    # fehlt cols → alle Spalten
                    affected_cols = rb.get('cols', None)
                    if affected_cols is None or ci in affected_cols:
                        for side in ('top', 'bottom', 'left', 'right'):
                            if side in rb:
                                border_cfg[side] = rb[side]
                elif ri in row_bdr and bb_sdef:
                    border_cfg['bottom'] = {
                        'style': 'single',
                        'sz': int(bb_sdef.border_bottom_pt * 8),
                        'color': bb_sdef.border_bottom_color
                    }
                if border_cfg:
                    self._set_cell_borders(cell, border_cfg)
    ...
    def _render_footer_from_blocks(self, doc, tpl, variables):
        """
        Rendert alle slot=footer Blöcke in den Word-Footer-Bereich.
        Nutzt _render_area_block — kein doppelter Code.
        """
        ...
        for section in doc.sections:
            ftr = section.footer
            # Trennlinie vor erstem Footer-Block
            if bb:
                p_sep = ftr.paragraphs[0] if ftr.paragraphs else ftr.add_paragraph()
                p_sep.clear()
                self._add_border(p_sep, bb.border_bottom_color,
                                 side='top', sz=int(bb.border_bottom_pt * 8))
            for tb in footer_blocks:
                block = tb.block
                # content_override berücksichtigen
                if tb.content_override:
                    block_content = block.content
                    block.content = tb.content_override
                self._render_area_block(doc, ftr, block, variables, 'footer', bb=None)
                if tb.content_override:
    ...
    def _render_header_footer(self, doc: DocxDocument,
        ...
        Nutzt _render_area_block um Block in Header/Footer-Area zu rendern.
        """
        ...
        for section in doc.sections:
            ...
            self._render_area_block(doc, area, block, variables, which, bb)

    def _render_area_block(self, doc, area, block, variables: dict,
                            which: str, bb) -> None:
        """
        Rendert einen Block in eine Header/Footer-Area.
        Unterstützt: FIELD, PARAGRAPH, FOOTER, DRAWING, TABLE, RAW_XML.
        """
        sk      = block.style_kit or doc._sk
        bt      = block.block_type
        ...
        if bt == 'FIELD':
            p_nr = area.paragraphs[0] if area.paragraphs else area.add_paragraph()
            p_nr.clear()
            ...
            if '	' in cnt:
                ...
                t1.set(_qn('w:pos'), str(int(((doc._lay.layout_refs or {}).get('header_tabs', {}).get('center_cm', (doc._lay.page_width_cm - doc._lay.margin_left_cm - doc._lay.margin_right_cm) / 2)) * 567)))
                tabs.append(t1)
                t2 = _OXE('w:tab')
                t2.set(_qn('w:val'), 'right')
                t2.set(_qn('w:pos'), str(int(((doc._lay.layout_refs or {}).get('header_tabs', {}).get('right_cm', doc._lay.page_width_cm - doc._lay.margin_left_cm - doc._lay.margin_right_cm)) * 567)))
                tabs.append(t2)
                pPr.append(tabs)
                ...
                for j, part in enumerate(parts):
                    ...
                    if '{PAGE}' in part or '{NUMPAGES}' in part:
                        sub = part.replace('{NUMPAGES}', '').split('{PAGE}')
            ...
            for i, line in enumerate(content.split('\n')):
                ...
                if '	' in line:
                    ...
                    if bt == 'FOOTER':
                        ...
                        self._add_soft_breaks(p, line.strip(), sdef)
                    else:
                        run = p.add_run(line.strip())
                        self._apply_style(p, skey, sk, run)
            ...
            sk2 = block.style_kit
            lay = doc._lay
            geo = (lay.layout_refs or {}).get(block.layout_ref or '', {})
            rendered = content
            lines = [l for l in rendered.split('\n') if l.strip()]
            if lines:
                col_widths_cm = geo.get('column_widths_cm', [])
                n_cols = max(len(col_widths_cm),
                             max((len(l.split('|')) for l in lines), default=1))
                if which == 'footer' and bb:
                    p_sep = area.paragraphs[0] if area.paragraphs else area.add_paragraph()
                    p_sep.clear()
                    self._add_border(p_sep, bb.border_bottom_color,
                                     side='top', sz=int(bb.border_bottom_pt * 8))
                total_w = sum(_cm(c) for c in col_widths_cm) or _cm(15.0)
                tbl = area.add_table(rows=len(lines), cols=n_cols, width=total_w)
                tbl.alignment = WD_TABLE_ALIGNMENT.LEFT
                col_styles = block.col_styles or []
                col_aligns = block.col_alignments or []
                for ri, line in enumerate(lines):
                    ...
                    for ci in range(n_cols):
                        ...
                        if text:
                            run = p.add_run(text)
                            self._apply_style(p, skey, sk2, run)
            ...
            xml_path   = os.path.join(
                ...
                'apps', 'abpe_doc_studio',
                'generator', 'templates',
                tpl_dir, f'{layout_ref}.xml'
            )
            if os.path.exists(xml_path):
                with open(xml_path, encoding='utf-8') as xf:
                    xml_str = xf.read()
                rendered_xml = self.variable_engine.render_text(xml_str, variables)
                try:
                    from lxml import etree as _etree
                    new_elem = _etree.fromstring(rendered_xml.encode('utf-8'))
                    for child in list(new_elem):
                        area._element.append(child)
                    if bb:
                        p_sep = area.paragraphs[0] if area.paragraphs else area.add_paragraph()
                        self._add_border(p_sep, bb.border_bottom_color,
                                         side='top', sz=int(bb.border_bottom_pt * 8))
                except Exception as e:
                    log.warning(f'Area RAW_XML Fehler: {e}')


    def _add_soft_breaks(self, paragraph, text: str, sdef) -> None:
        """Fuegt Text mit weichen Zeilenumbruechen (<w:br/>) ein."""
        parts = text.split('<w:br/>')
    ...
    def _render_textbox(self, doc, content: str, block) -> None:
        ...
        xml_path = os.path.join(
            ...
            'generator', 'templates',
            tpl_dir,
            f'{tpl_dir}_textbox_{layout_ref}.xml'
        )
        ...
        try:
            ...
            for rId in sorted(used_rids):
                ...
                matches = _glob.glob(os.path.join(media_base, f'*_image{num}.png'))
                img_path = matches[0] if matches else ''
    ...
    def _set_cell_borders(self, cell, border_config: dict) -> None:
        """
        Setzt Zellenränder aus Konfiguration.
        border_config: {
          "top":    {"style": "single", "sz": 4, "color": "163258"},
          "bottom": {"style": "double", "sz": 4, "color": "163258"},
          "left":   {"style": "nil"},
          "right":  {"style": "nil"}
        }
        ...
        tc   = cell._tc
        tcPr = tc.get_or_add_tcPr()
        for old_bdrs in tcPr.findall(qn('w:tcBorders')):
            tcPr.remove(old_bdrs)
        bdrs = OxmlElement('w:tcBorders')
        for side in ('top', 'left', 'bottom', 'right', 'insideH', 'insideV'):
            cfg = border_config.get(side, {'style': 'nil'})
            el  = OxmlElement(f'w:{side}')
            el.set(qn('w:val'),   cfg.get('style', 'nil'))
            if cfg.get('style', 'nil') != 'nil':
                el.set(qn('w:sz'),    str(cfg.get('sz', 4)))
                el.set(qn('w:space'), '0')
                el.set(qn('w:color'), cfg.get('color', '163258').lstrip('#'))
            bdrs.append(el)
        tcPr.append(bdrs)
