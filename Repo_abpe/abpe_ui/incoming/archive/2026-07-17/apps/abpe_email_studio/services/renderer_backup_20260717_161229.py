...
class EmailRenderer:
    ...
    def _resolve_modules_txt(
        ...
        def replace_module(match):
            ...
            if identifier == 'signature' and template is not None:
                ...
                return self._html_to_text(sig_html)
            module = EmailModule.objects.filter(
                identifier=identifier, is_active=True
            ).first()
    ...
    def render_html(self, template, variables: dict, user=None) -> str:
        ...
        if not self._has_signature_block(template.html_body):
            ...
            if sig:
                html = html + sig
        return self._wrap_font(self._strip_editor_markup(html))
    ...
    def render_text(self, template, variables: dict, user=None) -> str:
        ...
        if not self._has_signature_block(template.html_body, template.text_body):
            ...
            if sig:
                sig_txt = self._html_to_text(sig)
                if sig_txt:
    ...
    def render_preview(
        ...
        if txt_src:
            ...
            rendered_text = self._html_to_text(html)
        ...
        if not self._has_signature_block(html_src, txt_src):
            ...
            if sig_html:
                sig_text = self._html_to_text(sig_html)
                if sig_text:
                    rendered_text = (
        ...
        return {
            ...
            'html':    self._wrap_font(self._strip_editor_markup(self._strip_scripts(html))),
            'text':    rendered_text,
        }

    def _html_to_text(self, html: str) -> str:
        """Wandelt HTML in lesbaren Klartext um. Block-Elemente (p, div,
        li, h1-6, br, tr) werden zu Zeilenumbruechen, Inline-Elemente
        (strong, em, a, span ...) werden ohne Umbruch entfernt, damit
        Saetze nicht mitten im Wort zerreissen."""
        if not html:
            return ''
        text = html
        text = re.sub(r'<br\s*/?>', '\n', text, flags=re.I)
    ...
    def _wrap_font(self, html: str) -> str:
        """Wrapt das fertige HTML in einen durchgehenden Font-Rahmen,
        damit Outlook (Word-Rendering-Engine) nicht auf Times New Roman
        zurueckfaellt, wenn einzelne Textabschnitte keine eigene
        font-family-Angabe haben."""
        if not html:
            return ''
        return (
            '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">'
            '<tbody><tr><td style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#333;">'
            + html +
            '</td></tr></tbody></table>'
        )
    ...
    def _strip_editor_markup(self, html: str) -> str:
        """Entfernt interne Editor-Marker (Blockgrenzen des visuellen
        Editors), bevor die E-Mail angezeigt oder verschickt wird.
        Die Marker selbst bleiben im gespeicherten html_body erhalten,
        damit der Editor beim naechsten Laden die Bloecke zuverlaessig
        wiedererkennt (verhindert Div-Verschachtelung ueber die Zeit)."""
        if not html:
            return ''
        html = re.sub(r'\s*class="es-block[^"]*"', '', html)
        html = re.sub(r'\s*data-block-type="[^"]*"', '', html)
        return html
