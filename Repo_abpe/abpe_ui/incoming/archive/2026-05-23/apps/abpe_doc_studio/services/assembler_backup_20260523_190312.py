...
class DocAssembler:
    ...
    def _apply_header(self, doc: DocxDocument, layout, variables: dict) -> None:
        """
        Header wie Original-PDF:
          Zeile 1: Seitennummer rechts
          Zeile 2: 2-spaltige Tabelle — Links: Logo | Rechts: Kontaktdaten
          Trennlinie darunter
        """
        import os
        ...
        for section in doc.sections:
            ...
            # ── Zeile 2: Leer links | Logo + Kontakt rechts ───────────
            # Wie Original: Logo rechtsbündig neben dem Titel
            tbl = hdr.add_table(rows=1, cols=2, width=Cm(15.0))
            tbl.alignment = WD_TABLE_ALIGNMENT.LEFT
            lc = tbl.rows[0].cells[0]   # Links: leer
            rc = tbl.rows[0].cells[1]   # Rechts: Logo + Kontakt
            self._set_cell_width(lc, 5757)   # ~10cm leer
            self._set_cell_width(rc, 3881)   # ~6.9cm Logo+Kontakt
            ...
            # Rechts: Logo oben, dann Kontaktdaten
            # Logo
            rp_logo = rc.paragraphs[0]
            rp_logo.alignment = WD_ALIGN_PARAGRAPH.RIGHT
            rp_logo.paragraph_format.space_before = Pt(0)
            rp_logo.paragraph_format.space_after  = Pt(2)
            if logo_path:
                rr_logo = rp_logo.add_run()
                rr_logo.add_picture(logo_path, height=Cm(1.8))
            else:
                rr_logo = rp_logo.add_run('abcona e. K.')
                rr_logo.font.name = FONT_NAME
                rr_logo.font.size = Pt(14)
                rr_logo.bold      = True
                rr_logo.font.color.rgb = _rgb(COLOR_BRAND)

            # Kontaktdaten unter Logo
            contact = [
                (v.get('ag_firma',  'abcona e.K.'),                 True,  True,  9),
                ('active business consulting agency',                False, True,  7),
                (f"Tel: {v.get('ag_tel',  '+49 (0)6171 8867 00')}", False, False, 8),
                (f"Fax: {v.get('ag_fax',  '+49 (0)6171 8867 09')}", False, False, 8),
                (f"Mail: {v.get('ag_email','office@abcona.de')}",    False, False, 8),
                (f"http://{v.get('ag_web', 'www.abcona.de')}",       False, False, 8),
            ]
            for text, bold, italic, size in contact:
                ...
                rr.bold      = bold
                rr.italic    = italic
                rr.font.color.rgb = _rgb(COLOR_BRAND if bold else COLOR_TEXT)
    ...
    def _add_page_numbers(self, doc: DocxDocument, layout) -> None:
        """Veraltet — wird nicht mehr aufgerufen. Behalten für Kompatibilität."""
        pass
    ...
    def _render_logo(self, doc, variables: dict) -> None:
        """Logo ist im Header (_apply_header) — Body-Block ist no-op."""
        pass
