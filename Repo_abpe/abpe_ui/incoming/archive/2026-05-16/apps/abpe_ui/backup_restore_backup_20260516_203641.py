...
Aufruf (aus /opt/abpe/backend/):
    python apps/abpe_ui/backup_restore.py -save apps/abpe_ui/models.py
...
def add_to_index(original: str, backup_path: Path, timestamp: str, message: str):
    ...
    entry = {
        ...
        'backup_path':   rel,
        'timestamp':     timestamp,
        'date':          timestamp[:8][:4] + '-' + timestamp[:8][4:6] + '-' + timestamp[:8][6:],
...
def make_backup_path(original: str, timestamp: str) -> Path:
    """
    Erzeugt Backup-Pfad mit Original-Ordnerstruktur.
    apps/abpe_ui/models.py
    -> archive/2026-05-16/apps/abpe_ui/models_backup_20260516_143022.py
    """
    rel   = Path(original)
...
def do_cleanup(days: int):
    ...
    deleted_files = before - len(idx['backups'])
    save_index(idx)
    update_latest()

    head("Cleanup abgeschlossen")
    ok(f"{deleted_dirs} Ordner, {deleted_files} Index-Einträge entfernt")
