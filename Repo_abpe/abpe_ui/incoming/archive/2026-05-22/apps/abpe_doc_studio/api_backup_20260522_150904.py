...
class TemplateListCreateAPI(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if not layout:
            return JsonResponse({'error': f"Layout '{data['layout']}' nicht gefunden"}, status=404)

        style_kit = StyleKit.objects.filter(identifier=data['style_kit']).first()
        if not style_kit:
            return JsonResponse({'error': f"StyleKit '{data['style_kit']}' nicht gefunden"}, status=404)
...
class TemplateDetailAPI(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        data['blocks'] = list(
            ...
            .filter(template=tpl)
            .order_by('slot', 'order')
            .values('slot', 'order', 'block__identifier', 'block__name',
                    'block__block_type', 'style_override',
    ...
    def put(self, request, pk):
        ...
        updatable = ['name', 'description', 'scope', 'engine',
                     'status', 'variables', 'translation_languages']
...
class TemplateDuplicateAPI(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        # Blöcke mitkopieren
        for tb in DocTemplateBlock.objects.filter(template=src).order_by('slot', 'order'):
...
class TemplatePreviewAPI(LoginRequiredMixin, View):
    """
    Rendert eine Vorschau — gibt HTML-String oder DOCX-Bytes zurück.
    format=html  → JsonResponse mit html_preview
    format=docx  → FileResponse mit .docx
    format=pdf   → FileResponse mit .pdf (via LibreOffice)
    """
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            if fmt == 'html':
                html = assembler.preview_html(tpl, variables)
            ...
            return JsonResponse({'error': f'Unbekanntes Format: {fmt}'}, status=400)
...
class TemplateGenerateAPI(LoginRequiredMixin, View):
    """
    Generiert + speichert ein Dokument aus einem Template.
    """
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            # doc_log Objekt entfernen (nicht JSON-serialisierbar)
...
class GenerateAPI(LoginRequiredMixin, View):
    """
    Synchroner Aufruf — analog zu EmailStudio SendAPI.
    POST { template: "sub_dienstvertrag", variables: {...}, context_ref: "..." }
    """
...
class GenerateAsyncAPI(LoginRequiredMixin, View):
    """Asynchrone Generierung via Celery Queue."""
...
class BlockListCreateAPI(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return JsonResponse({'blocks': grouped, 'types': ContentBlock._meta.get_field('block_type').choices})
    ...
    def post(self, request):
        ...
        return JsonResponse({'id': block.pk, 'identifier': block.identifier}, status=201)
...
class InvoiceListCreateAPI(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        inv.save()
        return JsonResponse({'id': str(inv.id), 'invoice_number': inv.invoice_number}, status=201)
...
class InvoiceGenerateAPI(LoginRequiredMixin, View):
    """
    Generiert ein Rechnungsdokument aus einem InvoiceRecord.
    Wählt automatisch das richtige Template (zeitaufwand / arbeitspakete).
    """
    ...
    def post(self, request, pk):
        ...
        # Template-Identifier aus invoice_type ableiten
        template_map = {
        ...
        # Variablen aus InvoiceRecord bauen
        variables = {
            ...
            'summe_netto':       f'{inv.netto_euro:,.2f}'.replace(',', 'X').replace('.', ',').replace('X', '.'),
            'mwst_satz':         f'{inv.mwst_satz:.0f}',
            'mwst_euro':         f'{inv.mwst_euro:,.2f}'.replace(',', 'X').replace('.', ',').replace('X', '.'),
            'gesamtbetrag':      f'{inv.brutto_euro:,.2f}'.replace(',', 'X').replace('.', ',').replace('X', '.'),
        ...
        try:
            ...
            # DocLog mit Invoice verknüpfen
            if result.get('success') and result.get('log_id'):
                from .models import DocLog
                doc_log = DocLog.objects.filter(log_id=result['log_id']).first()
                if doc_log:
                    inv.doc_log = doc_log
...
class DocStudio:
    ...
    def generate(template: str,
        ...
        """
        Generiert ein Dokument aus einem Template.

        Returns:
            {
                'success': True,
                'log_id':  '...',
                'file_path_docx': '/data/doc_out/...',
                'file_path_pdf':  '/data/doc_out/...',
            }
        """
        tpl = DocTemplate.objects.filter(
    ...
    def render_bytes(template: str, variables: dict = None,
        ...
        """Rendert ein Template zu Bytes ohne zu speichern."""
...
class TemplateBlockInlineUpdateAPI(LoginRequiredMixin, View):
    """PUT /api/templates/<pk>/blocks/<tb_pk>/"""
    ...
    def put(self, request, pk, tb_pk):
        ...
        return JsonResponse({'success': True, 'id': tb.pk,
                             'block_id': b.pk, 'content': b.content})
