...
def api_email_templates(request):
    """Verfügbare E-Mail Vorlagen + Signaturen für CRM-Versand"""
    ...
    signatures = EmailSignature.objects.all().values('id', 'name', 'identifier', 'is_default')
    return JsonResponse({'templates': list(templates), 'signatures': list(signatures)})
...
def api_email_send(request):
    ...
    try:
        ...
        elif not signature_id:
            # Keine Signatur gewählt → keine anhängen
            tpl = copy(tpl)
