...
def _mm_resolve_signature(signature_id=None):
    ...
    import re as _re
    return _re.sub(r"<[^>]+>", "", sig.html_body or "").strip()
...
def api_guest_send_adhoc(request, guest_id):
    ...
    body_with_sig = _mm_append_signature(body, request.data.get('signature_id'))
...
def api_meeting_notify_bulk(request, meeting_id):
    ...
    body = _mm_append_signature(body, request.data.get('signature_id'))
