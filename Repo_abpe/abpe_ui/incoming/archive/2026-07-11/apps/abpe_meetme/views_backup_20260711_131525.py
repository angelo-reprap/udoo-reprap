...
def _mm_text_to_html(text):
    """Konvertiert reinen Text in einfaches HTML (Absaetze/Zeilenumbrueche)."""
...
def _mm_resolve_signature_html(signature_id=None):
    ...
    return _mm_text_to_html(sig.text_body or "")


def _mm_build_final_bodies(body, signature_id=None):
    ...
    html_final = _mm_text_to_html(body) + _mm_resolve_signature_html(signature_id)
...
def api_guest_send_adhoc(request, guest_id):
    ...
    text_final, html_final = _mm_build_final_bodies(body, request.data.get('signature_id'))
