...
class Command(BaseCommand):
    ...
    def _sync_accounts(self):
        ...
        for row in rows:
            ...
            if crm_id in existing_ids and not self.force:
                continue
            new_accounts.append(CrmAccount(
                crm_id                      = crm_id,
            ...
            new_cstms.append({'crm_id': crm_id,
                              'account_status_c': row['account_status_c'],
                              'kunden_nummer_c':  row['kunden_nummer_c']})

        created = 0
        for i in range(0, len(new_accounts), CHUNK):
            chunk = new_accounts[i:i+CHUNK]
            with transaction.atomic():
                CrmAccount.objects.bulk_create(chunk, ignore_conflicts=True)
            created += len(chunk)
            self.stdout.write(f'  ... {created}/{len(new_accounts)} Accounts')

        if new_cstms:
            cstm_objs = [CrmAccountCstm(
                account_id       = d['crm_id'],
                account_status_c = d['account_status_c'],
                kunden_nummer_c  = d['kunden_nummer_c'],
            ) for d in new_cstms]
            for i in range(0, len(cstm_objs), CHUNK):
                with transaction.atomic():
                    CrmAccountCstm.objects.bulk_create(cstm_objs[i:i+CHUNK], ignore_conflicts=True)

        self.stdout.write(self.style.SUCCESS(
            f'  Accounts -> neu: {created}  bereits vorhanden: {len(existing_ids)}'
        ))
    ...
    def _sync_contacts(self):
        ...
        for row in rows:
            ...
            if crm_id in existing_ids and not self.force:
                continue
            new_contacts.append(CrmContact(
                crm_id                      = crm_id,
                crm_date_entered            = row['date_entered'],
                crm_date_modified           = row['date_modified'],
        ...
        # Bulk insert Contacts in Chunks
        created = 0
        for i in range(0, len(new_contacts), CHUNK):
            chunk = new_contacts[i:i+CHUNK]
            with transaction.atomic():
                CrmContact.objects.bulk_create(chunk, ignore_conflicts=True)
            created += len(chunk)
            self.stdout.write(f'  ... {created}/{len(new_contacts)} Contacts')
        ...
        if new_cstms:
            ...
            for d in new_cstms:
                crm_id = d.pop('crm_id')
                cstm_objs.append(CrmContactCstm(contact_id=crm_id, **d))
            for i in range(0, len(cstm_objs), CHUNK):
                with transaction.atomic():
                    CrmContactCstm.objects.bulk_create(cstm_objs[i:i+CHUNK], ignore_conflicts=True)

        self.stdout.write(self.style.SUCCESS(
            f'  Contacts -> neu: {created}  bereits vorhanden: {len(existing_ids)}'
        ))
    ...
    def _sync_emails(self):
        ...
        cur.execute(sql)
        rows = cur.fetchall()
        self.stdout.write(f'  MySQL: {len(rows)} Email-Adressen (Contact-verknuepft)')
        ...
        for row in rows:
            ...
            if crm_id in existing_ids and not self.force:
                continue
            new_emails.append(CrmEmailAddress(
                crm_id                   = crm_id,
        ...
        for i in range(0, len(new_emails), CHUNK):
            chunk = new_emails[i:i+CHUNK]
            with transaction.atomic():
                CrmEmailAddress.objects.bulk_create(chunk, ignore_conflicts=True)
            created += len(chunk)
            self.stdout.write(f'  ... {created}/{len(new_emails)} Emails')

        self.stdout.write(self.style.SUCCESS(
            f'  EmailAddress -> neu: {created}  bereits vorhanden: {len(existing_ids)}'
        ))
    ...
    def _sync_email_bean_rel(self):
        ...
        cur.execute(sql)
        rows = cur.fetchall()
        self.stdout.write(f'  MySQL: {len(rows)} Bean-Relations')

        if self.dryrun:
            self.stdout.write(self.style.SUCCESS(f'  BeanRel -> neu: {len(rows)}  (dryrun)'))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f'  BeanRel -> neu: {created}'))
        if missing_email > 0:
            self.stdout.write(self.style.WARNING(
                f'  {missing_email} uebersprungen - Email nicht in PG'))
        if missing_contact > 0:
            self.stdout.write(self.style.WARNING(
                f'  {missing_contact} uebersprungen - Contact nicht in PG'))
