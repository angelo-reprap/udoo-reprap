...
(function (global) {
  ...
  function loadEinstellungen() {
    ...
    fetch(api('settings/'), { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
      ...
      .then(function (data) {
        ...
        renderSettingsEditor(host, rows);
      })
      .catch(function () {
        ...
        postSettings({ settings: settings })
          .then(function (d) {
            ...
            if (d && d.ok) {
              ...
              if (status) status.textContent = (d && d.error) || _t('sh.toast_error', 'Speichern fehlgeschlagen');
            }
          })
          .catch(function () {
            saveBtn.disabled = false;
  ...
  // Unter Regeln einstellbar (Zahlfelder). Termin = „kein Default“.
  ...
  function applyArtDueDefaults(art) {
    ...
    if (!def || !def.enabled) return; // Termin / kein Default
    ...
    if (dauerEl) {
      ...
      if (durMin > 0) {
        ...
        // nur Woche/Tag → Dauer leer lassen
        dauerEl.value = '';
      }
    }
  }
