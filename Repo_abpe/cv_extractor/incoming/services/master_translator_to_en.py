"""
master_translator_to_en.py
Prompt DB-Stage: master_translate_to_en
...
Singleton: master_translator_to_en
"""
import json
import logging
    ...
    def translate_pre_json(self, pre_json: dict, max_workers: int = 10) -> dict:
        ...
        def translate_field(args):
            ...
            if isinstance(value, list):
                return key, [_translate_to_en(str(i), context) for i in value]
        ...
        if ed.get('focus_experience'):
            # Extrahiere nur die 'name' Felder aus Dictionaries (falls vorhanden)
            fe = ed['focus_experience']
            if isinstance(fe, list) and len(fe) > 0 and isinstance(fe[0], dict):
                fe = [item.get('name', str(item)) for item in fe]
            field_tasks.append(('focus_experience', fe, 'Focus area'))
...
master_translator_to_en = MasterTranslatorToEn()
