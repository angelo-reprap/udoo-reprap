    ...
    _mmWizardRenderBody() {
        ...
        if (isSummaryTab) {
            ...
            return `
                ...
                <p class="pbx-hint" style="margin-top:10px">${this.t('pbx_wiz_summary_hint', 'Hinweis: Diese Zusammenfassung ist ein erster Entwurf - Betreff/Text werden noch nicht aus dem Compose-Reiter uebernommen (bekannter Bug, folgt).')}</p>
            `;
        }
...
Object.assign(PBX, {
    ...
    _mmNotifyDefaultText(m, g) {
        ...
        if (st.action === 'cancel') {
            return {
                subject: `Terminabsage: ${m.title}`,
                body: `Hallo ${g.name},\n\nder Termin "${m.title}" musste leider abgesagt werden.\n\nViele Grüße`,
            };
        }
        if (st.action === 'invite') {
            const fmtInv = this._meetmeFmtDateTime(st.newStartAt);
            return {
                subject: `Einladung: ${m.title}`,
                body: `Hallo ${g.name},\n\nich lade Sie herzlich zu folgendem Termin ein:\n\n${m.title}\n${fmtInv} Uhr\n\nViele Grüße`,
            };
        }
        const fmt = this._meetmeFmtDateTime(st.newStartAt);
        return {
            subject: `Terminänderung: ${m.title}`,
            body: `Hallo ${g.name},\n\nder Termin "${m.title}" wurde verschoben.\nNeu: ${fmt} Uhr\n\nViele Grüße`,
        };
    },
